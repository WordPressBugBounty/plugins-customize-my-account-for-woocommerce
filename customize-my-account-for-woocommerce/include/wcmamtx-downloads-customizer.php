...
define( 'WCMAMTX_DOWNLOADS_CUSTOMIZER_SLUG', 'wcmamtx_downloads_customizer' );

// ── Register menu page ──────────────────────────────────────────────────────
add_action( 'admin_menu', function() {
    add_menu_page(
        __( 'Downloads Customizer', 'customize-my-account-for-woocommerce' ),
        __( 'Downloads', 'customize-my-account-for-woocommerce' ),
        'manage_options',
        WCMAMTX_DOWNLOADS_CUSTOMIZER_SLUG,
        'wcmamtx_downloads_customizer_render_page',
        'dashicons-download',
        59
    );
}, 999 );
add_action( 'admin_head', function() { remove_menu_page( WCMAMTX_DOWNLOADS_CUSTOMIZER_SLUG ); } );

// ── Strip WP admin styles on this page ──────────────────────────────────────
add_action( 'admin_enqueue_scripts', function( $hook ) {
    if ( $hook !== 'toplevel_page_' . WCMAMTX_DOWNLOADS_CUSTOMIZER_SLUG ) return;
...
add_action( 'wp_ajax_wcmamtx_downloads_customizer_save', function() {
    check_ajax_referer( 'wcmamtx_downloads_customizer_nonce', 'nonce' );
    if ( ! current_user_can( 'manage_options' ) ) wp_send_json_error( 'Unauthorized' );
    $key   = isset( $_POST['key'] )   ? sanitize_key( $_POST['key'] ) : '';
    $value = isset( $_POST['value'] ) ? sanitize_text_field( wp_unslash( $_POST['value'] ) ) : '';
    if ( ! $key ) wp_send_json_error( 'Missing key' );
    // Only allow keys relevant to view-order
    $allowed = [ 'download_template_override', 'download_style' ];
...
// ── Render page ──────────────────────────────────────────────────────────────
function wcmamtx_downloads_customizer_render_page() {
    ...
    $nonce       = wp_create_nonce( 'wcmamtx_downloads_customizer_nonce' );
    $ajax_url    = admin_url( 'admin-ajax.php' );
    $back_url    = admin_url( 'admin.php?page=wcmamtx_frontend_customizer' );

    // Preview URL — use a real view-order page
    $preview_url = wc_get_account_endpoint_url( 'downloads' );

    // Current saved values
    $override    = $g( 'download_template_override', '01' );
    $style       = $g( 'download_style', '01' );
...
<title><?php esc_html_e( 'Downloads Live Customizer', 'customize-my-account-for-woocommerce' ); ?></title>
...
<div id="wcvo-cz">
    ...
    <div id="wcvo-topbar">
        <a href="<?php echo esc_url( admin_url( 'admin.php?page=wcmamtx_advanced_settings' ) ); ?>" class="cz-logo">
            ...
            <?php esc_html_e( 'SysBasics Downloads Live Customizer', 'customize-my-account-for-woocommerce' ); ?>
        </a>
    ...
    <div id="wcvo-body">
                        ...
                        <div class="cz-group">
                            ...
                            <div class="cz-field">
                                <div class="cz-toggle-group">
                                    <button class="cz-toggle cz-tpl-override <?php echo $override === '01' ? 'active' : ''; ?>" data-key="download_template_override" data-value="01"><?php esc_html_e( 'This Plugin', 'customize-my-account-for-woocommerce' ); ?></button>
                                    <button class="cz-toggle cz-tpl-override <?php echo $override === '02' ? 'active' : ''; ?>" data-key="download_template_override" data-value="02"><?php esc_html_e( 'No Override', 'customize-my-account-for-woocommerce' ); ?></button>
                                </div>
                            </div>
                            <div class="cz-field cz-tpl-style-wrap" style="<?php echo $override === '01' ? '' : 'display:none;'; ?>">
                                ...
                                <div class="cz-toggle-group">
                                    ...
                                    <button class="cz-toggle cz-tpl-style <?php echo $style === $sv ? 'active' : ''; ?>" data-key="download_style" data-value="<?php echo esc_attr( $sv ); ?>"><?php echo esc_html( $sl ); ?></button>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="padding:12px;">
                            ...
                            <div style="background:#0f0f1a;border:1px solid #2d2d3f;border-radius:8px;overflow:hidden;margin-bottom:10px;">
                                <div style="padding:10px 12px;border-bottom:1px solid #2d2d3f;">
                                    ...
                                    <p style="font-size:11px;color:#6b6b85;line-height:1.6;margin-bottom:8px;"><?php esc_html_e( 'Show or hide columns in the Downloads table — product, file, remaining and more.', 'customize-my-account-for-woocommerce' ); ?></p>
                                    <div style="display:flex;flex-direction:column;gap:5px;">
                                        <?php
                                        $dl_cols = [
                                            'dashicons-download'    => __( 'File Download Link',   'customize-my-account-for-woocommerce' ),
                                            'dashicons-products'    => __( 'Product Name',         'customize-my-account-for-woocommerce' ),
                                            'dashicons-backup'      => __( 'Downloads Remaining',  'customize-my-account-for-woocommerce' ),
                                            'dashicons-calendar-alt'=> __( 'Expiry Date',          'customize-my-account-for-woocommerce' ),
                                            'dashicons-cart'        => __( 'Order Reference',      'customize-my-account-for-woocommerce' ),
                                        ];
                                        foreach ( $dl_cols as $icon => $label ) : ?>
                                ...
                                <div style="padding:10px 12px;border-bottom:1px solid #2d2d3f;">
                                    ...
                                    <p style="font-size:11px;color:#6b6b85;line-height:1.6;margin-bottom:8px;"><?php esc_html_e( 'Reorder the columns of the Downloads table by dragging them into your preferred sequence.', 'customize-my-account-for-woocommerce' ); ?></p>
                                    <div style="display:flex;flex-direction:column;gap:4px;opacity:.5;">
                                        <?php
                                        $dl_order_rows = [
                                            __( 'File Download Link',  'customize-my-account-for-woocommerce' ),
                                            __( 'Product Name',        'customize-my-account-for-woocommerce' ),
                                            __( 'Downloads Remaining', 'customize-my-account-for-woocommerce' ),
                                        ];
                                        foreach ( $dl_order_rows as $row ) : ?>
                                ...
                                <div style="padding:10px 12px;">
                                    <p style="font-size:11px;font-weight:700;color:#a78bfa;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;"><?php esc_html_e( 'Custom Shortcode Block', 'customize-my-account-for-woocommerce' ); ?></p>
                                    <p style="font-size:11px;color:#6b6b85;line-height:1.6;margin-bottom:8px;"><?php esc_html_e( 'Add a shortcode block above or below the downloads table. Supports dynamic attributes:', 'customize-my-account-for-woocommerce' ); ?></p>
                                    <div style="background:#0d0d1a;border:1px solid #2d2d3f;border-radius:6px;padding:8px 10px;font-size:11px;color:#86efac;font-family:monospace;line-height:1.8;opacity:.6;">
                                        [my_plugin order_id="{order_id}"<br>
                ...
                <div class="cz-accordion" id="wcvo-override-accordion" data-accordion="override" style="<?php echo $override === '01' ? '' : 'display:none;'; ?>">
                    ...
                    <div class="cz-accordion-body">
                        <div class="cz-group">
                            ...
                            <code id="wcvo-tpl-source" style="display:block;background:#0d0d1a;border:1px solid #2d2d3f;border-radius:6px;padding:9px;font-size:11px;color:#86efac;line-height:1.6;word-break:break-all;margin-bottom:10px;"><?php echo esc_html( wcmamtx_PLUGIN_URL . 'templates/myaccount/order/downloads/' . $style . '.php' ); ?></code>
                            ...
                            <code id="wcvo-tpl-dest" style="display:block;background:#0d0d1a;border:1px solid #2d2d3f;border-radius:6px;padding:9px;font-size:11px;color:#fbbf24;line-height:1.6;word-break:break-all;"><?php echo esc_html( get_stylesheet_directory() . '/wcmamtx_template/order/downloads/' . $style . '.php' ); ?></code>
                        </div>
                    </div>
                </div>
        ...
        <div id="wcvo-preview-wrap">
            <div id="wcvo-preview-bar">
                <span class="dashicons dashicons-download" style="color:#a78bfa;font-size:16px;width:16px;height:16px;"></span>
...
(function(){
    ...
    function saveOption(key, value, reload) {
        ...
        var fd = new FormData();
        fd.append('action', 'wcmamtx_downloads_customizer_save');
        ...
        fetch(AJAX_URL, { method: 'POST', body: fd })
            ...
            .then(function(res){
                if (res.success) {
                    ...
                    if (reload) { showLoader(); iframe.src = PREVIEW + '?wco_dl=' + Date.now(); }
                } else {
    ...
    var pluginUrl   = <?php echo wp_json_encode( wcmamtx_PLUGIN_URL . 'templates/myaccount/order/downloads/' ); ?>;
    var themeDir    = <?php echo wp_json_encode( get_stylesheet_directory() . '/wcmamtx_template/order/downloads/' ); ?>;
    ...
    document.getElementById('cz-refresh-btn').addEventListener('click', function(){
        showLoader();
        iframe.src = PREVIEW + '?wco_dl=' + Date.now();
    });
